...
`./scripts/backup.sh` writes one timestamped archive and prints its path. It dumps whichever store
the deployment uses — the same `DATABASE_URL` switch the server makes at boot — and deletes nothing:

```bash
./scripts/backup.sh              # -> backups/doop-<UTC timestamp>.dump | .tar.gz
./scripts/backup.sh /mnt/backups # or an explicit output directory
```

- `DATABASE_URL` set: a `pg_dump --format=custom` dump, `backups/doop-<UTC timestamp>.dump`,
  restored with `pg_restore --clean --dbname "$DATABASE_URL" <file>`.
- `DATABASE_URL` unset: the embedded PGlite cluster tarred whole,
  `backups/doop-<UTC timestamp>.tar.gz` (the `data/pg` directory the server created).

**doop ships no scheduler**, so nothing runs this for you: put it on cron or a systemd timer and
rotate the files yourself — the script never prunes. With `docker compose`, Postgres lives in the
`db` container and is not published to the host, so dump it from there instead:

```bash
docker compose exec -T db pg_dump -U doop --format=custom doop > backups/doop-$(date -u +%Y%m%dT%H%M%SZ).dump
```
...
| `create_canvas_version`   | Freeze the whole canvas — every frame’s HTML, its geometry and stacking, and the design tokens — into its history, the manual “save version” of the History tab. The `label` rides the canvas activity feed; roll it back with restore_canvas_version.                                                                                                                   |
...
| `delete_canvas`           | Delete this canvas with its frames, pages, guides, references and comments (owner-only, confirm: true). It moves to the owner’s trash, where it can be restored for 30 days (the default).                                                                                                                                                                               |
| `delete_component`        | Remove a component from the library (confirm: true).                                                                                                                                                                                                                                                                                                                     |
| `delete_page`             | Deletes the page AND every frame on it, all to the trash. The canvas keeps ≥1 page — deleting the last one is refused. Rescue frames you still need with move_frame first.                                                                                                                                                                                               |
...
| `list_canvas_versions`    | The canvas’s checkpoints, newest first: the id to roll back to, why each was taken (a manual save, the automatic cadence, before a delete, or the state a restore replaced), when and by whom, and how many frames it froze.                                                                                                                                             |
...
| `list_trash`              | What this canvas has in the trash, newest first — the deleted frames, pages and components, each with the id that restores it, plus the page a frame would come back on and how many frames a page brings back with it. The trash is the owner’s view, exactly like the web UI’s.                                                                                        |
...
| `restore_canvas_version`  | Put a checkpoint back on the live canvas as ordinary writes: frames come back with their ids, frames added since are left alone, and the state it replaced is checkpointed itself. A frame another agent holds, or one the canvas’s review policy gates, is reported in `skipped` and left exactly as it is.                                                             |
| `restore_page`            | Put a deleted page back in the slot it held, and bring the frames that went down with it back too — a page delete is one action, so its inverse is one as well. Refused with `conflict` when another agent holds one of those frames.                                                                                                                                    |
...
| `set_link_access`         | Set the canvas’s share-link policy (owner-only): `view`, `comment`, `edit` or `none`.                                                                                                                                                                                                                                                                                    |
...
| `delete_frame`            | Delete a frame from its canvas — it moves to the trash and can be restored from there.                                                                                                                                                                                                                                                                                   |
...
| `get_frames_html`         | Read the source of several frames of one canvas in a single call — each the same bounded slice get_frame_html returns, with html_truncated when a document was cut. An id that is not on the canvas comes back as not_found rather than failing the read.                                                                                                                |
| `list_frame_locks`        | The frames an agent has claimed right now, with the holder and how long the claim has left — read it before a pass over several frames so you work the free ones first instead of hitting a conflict per write.                                                                                                                                                          |
...
| `reorder_frame`           | Change where a frame sits in its page’s stack — `front`, `back`, `forward` or `backward` — and get the page’s frames back, front to back. The fix for “the modal is behind the page”.                                                                                                                                                                                    |
| `restore_frame`           | Put a deleted frame back on its canvas with the id it always had, so its comments, its history and every reference to it still point at it. An ordinary edit: the room watches it appear, and it is logged and versioned. Refused with `conflict` when another agent holds the frame — `takeover: true` overrides.                                                       |
...
| `update_frame`            | Update frame metadata: rename it, move/resize it, or change how it presents (stacking z, locked, hidden, rotation, opacity).                                                                                                                                                                                                                                             |
...
| `edit_comment`            | Rewrite the text of a comment you wrote — the same note, not a new one: the id, the element anchor and its place in the thread all stay, so the replies under it keep their positions and a reader who already saw the note reads the correction. Only the author may edit a note; answer one that is not yours with reply_to_comment.                                   |
...
| `get_inbox`               | What is waiting for you across every canvas this account can reach: your open questions, unclaimed notes addressed to you or to the role you work, chat messages that reach you, and your proposals no human has decided yet.                                                                                                                                            |
| `get_messages`            | Read the canvas chat — your work queue: what the humans want done, oldest first. Pass `since` (the `at` of the last message you read) for only what is new.                                                                                                                                                                                                              |
...
| `unclaim_comment`         | Give back a note you claimed with claim_comment: its pin stops saying you are on it and another agent can pick it up. Only the claimant may release it, and a note nobody holds has nothing to release — a note you tried and could not finish is fail_comment’s job.                                                                                                    |
| `send_message`            | Post to the canvas chat: what you changed, what you need next, a question that is not worth blocking on. `to` routes it to an agent name or a role.                                                                                                                                                                                                                      |
| `wait_for_events`         | Block until something on this canvas needs you: a comment or @mention, an answer to your question, a proposal of yours being resolved, or a human taking over a frame you were streaming into. Pass `role` for the role you work — humans address work to roles, so a name that does not name one is woken only by @mentions of the name itself.                         |
| `withdraw_question`       | Take back an ask_human question you no longer need answered: it stops counting as open in the inbox and the Review panel instead of waiting out its time, and a parked ask_human is told it is settled. Only the agent that asked may withdraw it.                                                                                                                       |
...
| `get_frame_review`        | The verdict a frame already holds: the newest stored review — verdict, when and by whom, its blocking findings, and whether it still describes the frame as it stands. The same report the ship paths ask for, read back without re-rendering; it says so plainly when there is none.                                                                                    |
...
| `post_status`             | Write one line onto the run timeline the human watches — "step 3/7 rebuilding the hero" — so a stretch of work that makes no tool calls reads as thinking rather than stuck. It lands in your own run.                                                                                                                                                                   |
| `revert_run`              | Undo everything one run changed: each frame that run wrote goes back to the version it held before the run started. A frame written since, or locked by another agent, is skipped and reported, never clobbered.                                                                                                                                                         |
