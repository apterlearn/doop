...
describe('structured MCP errors', () => {
  ...
  it('reads a resource failure as the same structured payload a tool returns', async () => {
    seedCanvas()
    const { client, close } = await connect()
    try {
      await expect(client.readResource({ uri: 'doop://canvas/missing' })).rejects.toThrow(/not_found/)
    } finally {
      await close()
    }
  })
